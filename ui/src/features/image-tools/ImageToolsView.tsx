...
export function ImageToolsView({ scanId, assetIds, onAssetIdsChange }: Props) {
  ...
  const semanticQuery = useQuery({
    queryKey: [
      ...
      appSettings?.embedImageSearchThreshold ?? 0.24,
      appSettings?.embedImageDynamicEnabled ?? true,
      appSettings?.embedImageDynamicMargin ?? 0.05,
    ],
    queryFn: () =>
      semanticSearch({
        ...
        textThreshold: appSettings?.embedSearchThreshold ?? 0.4,
        imageThreshold: appSettings?.embedImageSearchThreshold ?? 0.24,
        imageDynamicEnabled: appSettings?.embedImageDynamicEnabled ?? true,
        imageDynamicMargin: appSettings?.embedImageDynamicMargin ?? 0.05,
        includeItems: true,
      }),
