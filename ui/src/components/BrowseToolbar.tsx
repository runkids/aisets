import { ArrowDownAZ, CheckSquare, Grid3X3, List, Moon, Search, Sun, Trees } from 'lucide-react'
import { useTranslation } from 'react-i18next'
import { Select, TextInput, type SegmentedControlItem } from './ui'
import { BrowseActionToggle, BrowseIconToggleGroup, BrowseSizeToggleGroup, BrowseStatusBar } from './BrowseToolbarParts'
...
export function BrowseToolbar({
  ...
  const viewItems: Array<SegmentedControlItem<ViewMode>> = [
    { value: 'grid', label: t('toolbar.gridView'), icon: <Grid3X3 size={16} /> },
  ...
  const bgItems = [
    { value: 'checker' as const, label: t('toolbar.checkerBg'), icon: <Grid3X3 size={16} /> },
    { value: 'light' as const, label: t('toolbar.lightBg'), icon: <Sun size={16} /> },
    { value: 'dark' as const, label: t('toolbar.darkBg'), icon: <Moon size={16} /> },
  ]
  const statusItems = [
  ...
  return (
    <div className="browse-toolbar">
      <div className="browse-toolbar-row">
        <BrowseIconToggleGroup value={view} items={viewItems} onChange={onViewChange} ariaLabel={t('tabs.viewAriaLabel')} />

        {view !== 'list' && (
          <BrowseSizeToggleGroup value={gridSize} items={sizeItems} onChange={onGridSizeChange} ariaLabel={t('toolbar.gridSize')} />
        )}

        <BrowseIconToggleGroup value={bgMode} items={bgItems} onChange={onBgModeChange} ariaLabel={t('toolbar.backgroundMode')} />

        <Select value={sortMode} options={sortItems} onChange={(value) => onSortChange(value as SortMode)} aria-label={t('toolbar.sort')} />

        <BrowseActionToggle active={bulkMode} label={t('toolbar.bulkSelect')} onToggle={onBulkToggle}>
          <CheckSquare size={16} />
        </BrowseActionToggle>
      ...
      <BrowseStatusBar value={statusFilter} items={statusItems} onChange={onStatusFilterChange} ariaLabel={t('toolbar.statusFilter')} />
    </div>
  )
}
