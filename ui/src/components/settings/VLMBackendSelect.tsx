...
function parseBackendValue(value: string): ParsedBackend {
  ...
  if (value.startsWith("agent:")) {
    ...
    if (idx >= 0) {
      return { type: "agent", adapterId: rest.slice(0, idx), model: rest.slice(idx + 1) };
    }
...
export function VLMBackendSelect({
  ...
  const adapters = agentRuntime?.adapters?.filter((a) => a.id !== "local-llm") ?? [];
  ...
  function handleAgentModelCommit() {
    ...
    const next = formatBackendValue({ type: "agent", adapterId: parsed.adapterId, model: trimmed });
    if (next !== value) {
      onChange(next);
    }
  }
          ...
          >
            ...
            {(localModels.length > 0 || isConnected) && (
              <DropdownMenuPrimitive.Group>
                ...
                {!showInherit && <DropdownMenuPrimitive.Item
                ...
                >
                  ...
                  <span className="min-w-0 flex-1 truncate">
                    ...
                    {llmRuntime?.visionModel ? ` (${llmRuntime.visionModel})` : ""}
                  </span>
