...
- For layout tasks, tool execution evidence is necessary but not sufficient. If the request asks for a row, grid, equal spacing, alignment, size, layering, or no overlap, inspect the final projected or visible geometry and assert the actual card boxes satisfy the request before marking the case pass.
