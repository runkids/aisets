...
- **Native build dependencies must match CI, not just the base image.** imgtools' `avif-native` feature pulls `dav1d-sys`, whose `system-deps` metadata requires libdav1d `>= 1.3.0`. CI runs on Ubuntu (new enough), but the devcontainer is Debian bookworm, which ships libdav1d `1.0.0` — so a clean `cargo build` of imgtools fails in the container while CI passes. `docker/sandbox/Dockerfile` installs `nasm meson ninja-build pkg-config` and sets `ENV SYSTEM_DEPS_BUILD_INTERNAL=auto` so system-deps compiles dav1d from source instead of relying on Debian's old apt package. When adding any Rust crate that links a system library via pkg-config / system-deps, check whether the bookworm package meets the required version; if not, add the build toolchain and let system-deps build from source rather than dropping the feature.
- **Same Docker image does not guarantee the same build result.** The Rust `target/` and other build caches live on each contributor's host through the `${PROJECT_ROOT}:/workspace` bind mount, not inside the image or a named volume. A stale `target/` can serve a binary built before a dependency changed, so one machine builds clean while another hits a fresh-build failure on the same image. After changing imgtools' `Cargo.toml` or its system deps, verify with a forced rebuild (`cargo clean`, or remove `tools/imgtools/target`) inside a freshly rebuilt image. Note `make devc` only reruns `setup.sh` (which builds imgtools) on first init; on an already-initialized container it runs `start-dev.sh` and skips the rebuild, so use `make devc-reset` to force a full re-setup.
