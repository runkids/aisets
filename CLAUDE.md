...
- **Canvas VLM repair loops must use deterministic compact follow-up prompts.** The first Canvas VLM round can include the full prompt, images, and canvas state, but later rounds must be built by backend code from the original user request, explicit loop reason, relevant card IDs/positions, compact tool results, and necessary previous text. Do not append raw tool results or ask the model to summarize loop state; prompt payloads grow quickly and can make tool-use repairs slower and less reliable.
