...
- **Canvas captures must render every image-card source.** Screenshot/export and AI snapshot code must collect rendered frames from catalog asset cards, uploaded image cards, and generated variant cards through a shared image-frame marker, not asset-only selectors. Otherwise visible image variants such as rotated or mirrored PNG cards can disappear from captures and AI visual context.
- **Canvas photo-staging transforms must stay art-directed and arbitrary-angle capable.** Photo-staging prompts and harnesses should tell agents that `rotate_image` supports any integer-degree angle, while still treating rotation and mirroring as optional composition tools for a few deliberate variants. Do not constrain staging prompts back to 90/180/270 degrees, and do not force transforms merely to demonstrate tool coverage.
...
- **Canvas follow-up prompts must use projected tool state.** When a loop executes state-changing tools such as `resize_card`, `move_card`, `arrange_cards`, or `bring_cards_to_front`, the next follow-up must reason from a projected current canvas state derived from those tool results, not from the original request snapshot. Otherwise the model can complete a missing later tool while re-planning geometry against stale card positions, widths, or layers.
...
- **Canvas resize cursors must anchor to the rendered handle.** During manual resize and AI-driven resize animations, position the cursor tip on the actual bottom-right resize handle in rendered canvas coordinates, accounting for viewport scale and any screen-stable card scale. Placing the cursor at the unadjusted card edge makes it drift away from the tiny handle when the card or camera is zoomed.
