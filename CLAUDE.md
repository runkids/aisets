...
- **AI/OCR filter queries must scope to the current model.** `ocr_results` and `ai_tags` unique keys include `engine_version` / `provider_name` + `model_name`. Changing models creates new rows, not overwrites. Facet counts (`catalogVLMOcrReadyCount`, `catalogAITagReadyCount`) and WHERE clause filters (`vlmOcrReady`, `aiTagReady`, etc.) must filter by the current `engine_version` / provider+model — otherwise stale 'ready' records from previous models inflate counts and break filtering. The badge display path (`enrichCatalogOCR`) already scopes correctly; filter/facet code must match.
